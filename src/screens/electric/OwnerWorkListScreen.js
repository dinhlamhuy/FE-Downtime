...
const OwnerWorkListScreen = () => {
  ...
  const validate = (values) => {
    ...
    if (
      new Date(format(DateTo.$d, "yyyy-MM-dd")) <
      new Date(format(DateFrom.$d, "yyyy-MM-dd"))
    ) {
      error.DateFrom = " ";
      error.DateTo = " ";
  ...
  useEffect(() => {
    if (alertValidate && alertCount >= 2) {
      Toast.fire({
        icon: "error",
        title: t("personal_info.validate_date_from"),
      });
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      await dispatch(
        get_list_status_task_detail({ fromdate, todate, factory, floor, lean })
      );
      await dispatch(
        get_count_status_task({ fromdate, todate, factory, floor, lean })
      );
      await dispatch(
        get_owner_task_damage({
          factory,
  ...
  const breadCrumbText =
    value === 0 ? t("work_list.work_list") : t("repair_status.repair_status");
    ...
    <Box component="div">
      ...
      <Box component="div" sx={{ display: "block", margin: "0 auto" }}>
        ...
        <CustomTabPanel value={value} index={1}>
          <RepairStatus
            fromDate={formik.values.DateFrom}
            toDate={formik.values.DateTo}
          />
        </CustomTabPanel>
      </Box>
